<?php
    ...
    if (isset($_POST['submit'])) {
      ...
      $approved_numb = $_POST['approved_numb'];
      ...
      try {
        ...
        if ($count == 0) {
          $insert = $dbconn->prepare("INSERT INTO area_assessment_points (area_number,under_area,user_id,year_,date_,time_,trail,approved$approved_numb) VALUES (?,?,?,?,?,?,?,?)");
          ...
          $insert->bindParam(8, $approve);
          $insert->execute();
        }else{
          $update = $dbconn->prepare("UPDATE area_assessment_points SET approved$approved_numb=? where user_id=? and year_=? and area_number=? and under_area=?");
          $update->bindParam(1, $approve);
